...
export default class SIPBuddy {
  ...
  constructor(
    ...
    this.iceServer =
      iceServer && iceServer !== '' ? JSON.parse(iceServer) : null;
  ...
  outgoingCallEventMiddleware(event, data) {
    ...
    switch (event) {
      ...
      case CALL_STATUS.FAILED:
        if (
          data.cause === 'User Denied Media Access' &&
          this.getUserMediaRetries > 0
        ) {
        ...
        console.log(
          `SIPBuddy::: Outgoing Call Event: ${event} Status Code: ${statusCode}`
        );
  ...
  incomingCallEventMiddleware(event, data) {
    ...
    switch (event) {
      ...
      case CALL_STATUS.FAILED:
        ...
        statusCode = this.getSipCode(data);
        console.log(
          `SIPBuddy::: Incoming Call Event: ${event} Status Code: ${statusCode}`
        );
  ...
  toggleMute() {
    ...
    if (this.session.isMuted().audio)
      this.session.unmute(this.mediaConstraints);
  ...
  reInitiateRegistration() {
    if (!(this.isSessionRegistered() && this.isWebSocketConnected()))
      this.userAgent.start();
  }
